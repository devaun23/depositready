...
export function Step6ReviewPay() {
  const { data, updateData } = useLandlordIntake();
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState("");

  const price = data.tier === "complete" ? 199 : 129;
  ...
  const threatLabel =
    data.threatType === "demand_letter"
      ? "Demand Letter"
      : data.threatType === "attorney_letter"
        ? "Attorney Letter"
        : data.threatType === "court_filing"
          ? "Court Filing"
          : data.threatType === "email"
            ? "Email / Text"
            : data.threatType === "verbal"
              ? "Verbal Threat"
              : "Unknown";
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        >
          ...
          <ul className="text-xs text-gray-600 space-y-1">
            <li>&#10003; Defense strategy report</li>
            <li>&#10003; Liability exposure analysis</li>
        ...
        >
          ...
          <ul className="text-xs text-gray-600 space-y-1">
            ...
            <li>&#10003; Settlement negotiation guide</li>
            <li>&#10003; Court preparation kit</li>
            <li>&#10003; Evidence organization checklist</li>
          </ul>
        </button>
      </div>
      ...
      <div className="bg-gray-50 rounded-lg p-4 text-sm space-y-1">
        <div className="flex justify-between text-gray-700">
          <span>Defense Kit ({data.tier === "complete" ? "Complete" : "Standard"})</span>
          <span className="font-semibold">${price}</span>
        </div>
        {data.stateCode && (
          <p className="text-xs text-gray-500">State: {data.stateCode}</p>
        )}
        {data.threatType && (
          <p className="text-xs text-gray-500">Threat: {threatLabel}</p>
        )}
        {data.tenantName && (
          <p className="text-xs text-gray-500">Tenant: {data.tenantName}</p>
        )}
      </div>
      ...
      >
        {isLoading
          ...
          : `Pay $${price} — Get Your Defense Kit`}
      </button>
