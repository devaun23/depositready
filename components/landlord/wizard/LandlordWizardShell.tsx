...
import {
  useLandlordWizard,
  LANDLORD_WIZARD_STEPS,
} from "./LandlordWizardContext";

interface LandlordWizardShellProps {
  children: React.ReactNode;
  onComplete?: () => void;
}

export function LandlordWizardShell({
  children,
  onComplete,
}: LandlordWizardShellProps) {
  const {
    currentStep,
    totalSteps,
    nextStep,
  ...
  const currentStepInfo = LANDLORD_WIZARD_STEPS[currentStep - 1];
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-3xl mx-auto px-4 py-2 md:py-3">
          <div className="flex gap-1 sm:gap-0 sm:justify-between overflow-x-auto scrollbar-hide">
            {LANDLORD_WIZARD_STEPS.map((step) => (
              <div
                key={step.id}
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6 md:p-8">
          ...
          <div className="mt-8 pt-6 border-t border-gray-200">
            ...
            <div className="flex justify-between">
              ...
              >
                {isLastStep ? "Get Response Kit — $79" : "Continue"}
              </button>
            </div>
          </div>
        </div>
