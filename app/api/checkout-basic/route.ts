...
const PRICE_CENTS = 2900; // $29.00 for basic recovery kit
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { tenantName, tenantEmail, propertyAddress, depositAmount, formData, source } = body;
    ...
    const { data: order, error: dbError } = await supabaseAdmin
      ...
      .insert({
        ...
        product_type: "basic", // Distinguish from full package
      })
      .select()
      .single();
    ...
    // Create Stripe checkout session for $29 product
    const session = await stripeFetch.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            currency: "usd",
            product_data: {
              name: "Security Deposit Recovery Kit",
              description: "State-specific demand letter + evidence checklist",
            },
      ...
      metadata: {
        order_id: order.id,
        product_type: "basic",
        source: source || "free_upsell",
        tenant_name: tenantName,
        property_address: propertyAddress,
        deposit_amount: depositAmount?.toString() || "0",
      },
      success_url: `${baseUrl}/success?session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${baseUrl}/free?canceled=true`,
      ...(tenantEmail && { customer_email: tenantEmail }),
      allow_promotion_codes: true,
    });

    // Update order with stripe session ID (non-blocking — webhook resolves via metadata)
    supabaseAdmin
      .from("orders")
    ...
    console.error("Basic checkout session error:", error);
